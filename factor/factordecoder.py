...
from keras.layers import Dense, Reshape, Conv2DTranspose, BatchNormalization, Dropout, Input
...
class decoder(keras.layers.Layer):
    ...
    def call(self, inputs):
        #inputs = Input(shape=(None, 10), dtype=tf.float32)
        inputs = tf.cast(inputs, dtype=tf.float32)
        x_rec = self.layer_1(inputs)
        #x_rec = self.dropout_1(x_rec)
...
#data = dsprite()
#x_test = data.x_train[0:10]
#enc = encoder(10)
#z, z_mean, z_log_var = enc(x_test)
#dec = decoder()
#abs = dec(z)
#print(dec.trainable_weights)
#x_hat = dec(z)
#print(x_hat.shape)
#print(enc.trainable_weights)