...
class factorvae(keras.Model):
    ...
    def generate_loss(self, data):
        ...
        x_rec = self.decoder(z)
        bce = tf.keras.losses.BinaryCrossentropy(reduction=tf.keras.losses.Reduction.NONE) #reduction=losses_utils.ReductionV2.AUTO
        ...
        tc_reg = self.gamma * tf.reduce_mean(tf.math.log(probs_true[:, 0]/(probs_true[:, 1])))
