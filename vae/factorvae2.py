...
class factorvae(keras.Model):
    ...
    def generate_loss(self, data):
        ...
        logits_true, probs_true = self.disc(z)
        logits_permuted, probs_permuted = self.disc(z_permuted)
        x_rec = self.decoder(z)
        bce = tf.keras.losses.BinaryCrossentropy() #reduction=losses_utils.ReductionV2.AUTO
        ...
        tc_reg = self.gamma * tf.reduce_mean(logits_true[:, 0] - logits_true[:, 1])
        ...
        #tf.add(0.5 * tf.reduce_mean(tf.log(probs_real[:, 0])), 0.5 * tf.reduce_mean(tf.log(probs_permuted[:, 1])),
...
#spara allt, så vi behöver recon_error, KL, tc_reg, discloss m.m
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            grads_vae = tape.gradient(total_loss, self.trainable_weights)
            self.vae_optimizer.apply_gradients(zip(grads_vae, self.trainable_weights))
