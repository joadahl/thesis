...
def loss_betavae():
    #här borde jag lägga in recon error
    loss = []
    for i in range(len(trained_models)):
        data = np.genfromtxt("betavae" + "lat" + str(trained_models[i]) + "weight" + str(4) + "/history/lossvae.csv",
                             delimiter=",",
                             names=["epoch", "loss", "recon_error", "KL", "val_loss", "val_recon_error",
                                    "val_KL"])
        loss.append(data["loss"][-1])
    plt.plot(trained_models, loss, '-o', label="Betavae," + " " + r"$\beta=$" + str(4))#label=("BetaVAE,"+ r"$\beta=$" + str(4)))
    plt.legend()
    ax = plt.gca()
    ax.set_xticks(trained_models)
    plt.ylabel("Loss")
    plt.xlabel(r"$D$")
    plt.show()
...
def plot_dims_metric_models(model, weight):
    trained_models_beta = [4, 6, 8, 10, 15]
    score_store = np.zeros((3, len(trained_models_beta)))
    if model == "betavae":
        for step, value in enumerate(trained_models_beta):
    ...
    plt.plot(trained_models_beta, score_store[1], '-o', label="factorscore")
    plt.plot(trained_models_beta, score_store[2], '-o', label="dciscore")  # vi saknar betavaescore på grund av tabbe
    #plt.bar(trained_models_beta, score_store[0], label="betascore")
    #plt.bar(trained_models_beta, score_store[1], label="factorscore")
    #plt.bar(trained_models_beta, score_store[2], label="dciscore")
    ax = plt.gca()
    plt.xlabel(r"$D$")
    plt.ylabel("Accuracy score in %")
    ax.set_xticks(trained_models_beta)
    ax.set_yticks(np.arange(0, 1.1, 0.1))  # np.arange(self.latent_dims)
    ax.get_xaxis().set_visible(True)
    ax.get_yaxis().set_visible(True)
    ax.legend()
    plt.show()
...
#data = pd.read_csv("betavaelat4weight4" + "/history/scores.csv", delimiter=",")
#a = np.array(data["betascore"].dropna())
#b = np.array(data["factorscore"].dropna())
#print(b)
#plot_loss_model_betavae(10, 8, "epoch", "loss")
#hinton("betavae", 10, 4)
...
#plot_dims_metric_models("betavae", 4)
#loss_betavae()
df = pd.read_csv("betavae" + "lat" + str(10) + "weight" + str(8) + "/history/scores.csv", delimiter=",")
print(np.array(df["DCIscore"].dropna()))