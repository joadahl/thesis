...
def plot_loss_model_betavae(lat_dims, x_axis, y_axis):
    data = np.genfromtxt("betavae" + str(lat_dims) + "/history/lossvae.csv", delimiter=",",
...
def plot_loss_model_factorvae(lat_dims, x_axis, y_axis):
    data = np.genfromtxt("factorvae" + str(lat_dims) + "/history/lossvae.csv", delimiter=",",
                         names=["epoch", "loss", "disc_loss", "recon_error", "KL", "tc_reg", "val_loss",
                                "val_disc_loss",
                                "val_recon_error", "val_KL", "val_tc_reg"])

    plt.plot(data[x_axis], data[y_axis], label="factorvae"+","+ r"$D=$" + str(lat_dims))
    plt.xlabel(x_axis)
    plt.ylabel(y_axis)
    # plt.title('fillin')
    plt.legend()
...
plot_loss_model_betavae(10, "epoch", "KL")
#hinton("betavae", 10)
