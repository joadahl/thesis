...
class cbbeta(keras.callbacks.Callback):
    def __init__(self, path):
        ...
        self.path = path
        #self.checkpoint_dir = self.path + "\modelstore"
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if min(self.epoch_val_loss.values()) == self.epoch_val_loss[epoch] and epoch != 0: #min(self.epoch_val_loss, key=d.get):, denna verkar typ funka
            #checkpoint_dir = self.path + "\modelstore"
        ...
        if epoch % 5 == 0:
            ...
            #self.model.vae.save_weights(self.path + '/modelstore/betavae' + str(epoch) + '.ckpt')  # denna används för vikter
