...
class loadmodel:
    def __init__(self, latent_dims, model, weight, seed):
        self.data = dsprite(seed)
        self.model = model
        self.weight = weight
        self.seed = seed
        self.latent_dims = latent_dims
        self.path = self.model + "lat" + str(self.latent_dims) + "weight" + str(self.weight) + "seed" + str(seed)
        ...
        if self.model == "betavae":
            self.network = self.make_or_restore_beta_vae(self.weight, self.seed)
            self.callback = cbbeta(self.path)
        if self.model == "factorvae":
            self.network = self.make_or_restore_factor_vae(self.weight, self.seed)
            self.callback = cbfactor(self.path)


    def make_or_restore_beta_vae(self, weight, seed):
        network = betavae(self.latent_dims, weight, seed)
    ...
    def make_or_restore_factor_vae(self, weight, seed):
        network = factorvae(self.latent_dims, weight, seed)
    ...
    def train_vae(self, epochs_n):
        ...
        with tf.device('/gpu:0'):
            history = self.network.fit(self.data.x_train, self.data.x_train, validation_data=(self.data.x_test, self.data.x_test),
                                          batch_size=64, verbose=0, epochs=ini_epoch + epochs_n, initial_epoch=ini_epoch, callbacks=[self.callback])
    ...
    def DCI_score(self, amount_n, L):
        ...
        for j in range(len(self.data.gen_factor_size) - 1):
            clf = GradientBoostingClassifier(n_estimators=10, random_state=self.seed)  # använder classifier
...
t = loadmodel(4, "betavae", 4, 0) #lower lr for this
t.train_vae(10)
